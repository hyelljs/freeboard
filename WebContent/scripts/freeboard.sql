...
insert into freeboard (name,password,subject,content,ip)
values ('세리박','1111','하이 ~~','운동하러 가야지',null);
...
select * from freeboard f where idx=159 and password ='1212';   -- 옳바른 비밀번호 : 쿼리결과 1개 행 조회
...
select count(*) from board_comment where mref=100;      -- 100번글의 댓글 갯수
...
-- 한페이지의 모든 행을 update 
update freeboard set commentCount=0;  -- 테스트 
-- update join쿼리
update freeboard f 
...
-- 로그인 sql
select * from customer where email = "korea@naver.com" 
and password="1111";  -- 틀린 패스워드 (결과 null)

select * from customer where email = "kkkorea@naver.com" 
and password="1111";  -- id가 없는 사용자 (결과 null)

select * from customer where email = "korea@naver.com" 
and password="22222222"; -- 맞는 패스워드 (결과 not null)
