...
public class GalleryRegistAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			//업로드된 파일을 받을 수 있는 request 객체 생성.
			MultipartRequest multi_request = new MultipartRequest(request,path,size,"UTF-8",
												new DefaultFileRenamePolicy());
			//기존 파일과명 중복된 파일명은 이름을 변경 -> 기본방식은 파일명 뒤에 순차적으로 번호 부여
			//title(text), pic(file) 2개 파라미터
			title= multi_request.getParameter("title");
			filename = multi_request.getFilesystemName("pic");
			//파일를 받아오고 지정된 path에 저장, 저장된 파일명 리턴
